...
def filter_state_dict(dict, filter_callback):
    new_dict = collections.OrderedDict()
    for _ in range(len(dict)):
        key, val = filter_callback(dict.popitem(False))
        if key is not None:
            new_dict[key] = val
    return new_dict
...
def flow_resnet34_from_rgb(rgb_model_path, pretrained=False, channels=20, num_classes=61):
    """Constructs a ResNet-34 model.
    ...
    if pretrained:
        in_channels = channels
        pretrained_dict = model_zoo.load_url(model_urls['resnet34'])
        rgb_dict = filter_state_dict(torch.load(rgb_model_path), lambda item: (item[0].replace('resNet.', ''), item[1]))
        # pretrained_dict.update(rgb_dict)
        for key in pretrained_dict.keys():  # retrieve rgb model weights intstead
            pretrained_dict[key] = rgb_dict[key]
        model_dict = model.state_dict()
